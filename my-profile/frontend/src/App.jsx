...
function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      const fetchDb = fetch('http://localhost:3000/api/guestbook')
        .then(res => {
          if (!res.ok) return []; 
          return res.json();
        })
      ...
      await Promise.all([minimumLoadTime, fetchDb]);
      setAppLoading(false);
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newData = await fetch('http://localhost:3000/api/guestbook')
        .then(res => res.json())
        .catch(() => []);
        
      setEntries(Array.isArray(newData) ? newData : []);
          ...
          <div className="brutal-container guestbook-container">
            ...
            <div className="guestbook-grid">
              <div className="guestbook-form-area">
                <h2>Leave a Mark</h2>
                <p className="subtitle">Sign my digital guestbook. Connected via Nest.js & Supabase.</p>
